...
export default function PassportPage() {
    ...
    return (
        <>
            <Topbar title="Conference Passport" actions={<button className="topbar-action accent" onClick={() => router.push('/app/scan')}>📷 Scan</button>} />
            <div className="content">
                <div className="wallet-card">
                    <div style={{ fontFamily: 'var(--fh)', fontWeight: 800, fontSize: 22, marginBottom: 4 }}>My Passport</div>
                    <div style={{ fontSize: 13, color: 'var(--sub)', marginBottom: 16 }}>Scan booth QR codes to collect stamps</div>
                    <div style={{ marginBottom: 4 }}>
                        ...
                        <div className="prog-wrap"><div className="prog-bar" style={{ width: totalAll ? `${Math.round(totalDone / totalAll * 100)}%` : '0%', background: 'var(--accent)' }} /></div>
                    </div>
                    {allDone && <div style={{ marginTop: 10, fontSize: 13, color: 'var(--green)', fontWeight: 600 }}>🎉 Passport complete!</div>}
                </div>
                {loading && <Loader />}
                {rooms.map(room => {
                    ...
                    return (
                        <div key={room.id} style={{ background: 'var(--s1)', border: '1px solid var(--border)', borderRadius: 'var(--r)', padding: 16, marginBottom: 12 }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                                <div style={{ fontFamily: 'var(--fh)', fontWeight: 700, fontSize: 16 }}>{room.emoji} {room.label}</div>
                                <span style={{ fontSize: 12, color: 'var(--sub)' }}>{done}/{room.booths.length}</span>
                            </div>
                            <div className="prog-wrap" style={{ marginBottom: 14 }}><div className="prog-bar" style={{ width: `${pct}%`, background: room.color }} /></div>
                            {room.booths.length > 0 ? (
                                ...
                                <div style={{ fontSize: 12, color: 'var(--muted)', textAlign: 'center', padding: 12 }}>No booths in this room yet</div>
                            )}
                        </div>
                    )
                })}
