...
export default function ScanPage() {
  ...
  async function process() {
    ...
    try {
      if (raw.startsWith('BOOTH-')) {
        await handleBooth(raw.replace('BOOTH-', ''))
      } else if (raw.startsWith('PITCH-')) {
        setResult({ info: '🏆 Navigate to the Pitch Room and find this company to vote!' })
      } else {
        await handleBusinessCard(JSON.parse(raw))
      }
    } catch {
      setResult({ error: '❌ Invalid QR code — try again.' })
    }
  ...
  async function handleBooth(companyId) {
    ...
    const { data: existing } = await sb.from('passport_stamps').select('id')
  ...
  async function handleBusinessCard(data) {
    ...
    const { data: existing } = await sb.from('collected_cards').select('id')
      .eq('collected_by', profile.id).eq('email', data.email).maybeSingle()

    if (existing) { setResult({ info: `Card already saved: ${data.name}` }); return }

    await sb.from('collected_cards').insert({
  ...
  async function checkRaffle() {
    const [{ data: stamps }, { data: votes }, { data: rooms }] = await Promise.all([
      ...
      sb.from('votes').select('id').eq('voter_id', profile.id).limit(1),
      sb.from('companies').select('id,room_type').in('room_type', ['poster_room', 'conference_room']),
    ])
    ...
    if (posterTotal > 0 && posterDone >= posterTotal && confDone >= confTotal && voted) {
      await sb.from('raffle_entries').upsert({
        user_id:    profile.id,
        email:      profile.email,
    ...
    <>
      ...
      <div className="content">
        <div className="card accent" style={{ marginBottom: 16, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 14, fontFamily: 'var(--fh)', fontWeight: 700, marginBottom: 4 }}>
            Camera Scan
          </div>
          <div style={{ fontSize: 13, color: 'var(--sub)', lineHeight: 1.5 }}>
            Camera scanning requires HTTPS deployment.<br />
            Use manual paste below during development.
          </div>
        </div>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            rows={4}
            placeholder={'BOOTH-{id}  or  PITCH-{id}  or  {"name":"Jane","email":"…"}'}
            value={code}
        ...
        {result && (
          <div className="card" style={{ marginTop: 16, borderColor: resultBorder }}>
            <div style={{ fontSize: 14, color: resultColor }}>
              {result.error || result.success || result.info}
            </div>
          </div>
        )}
      </div>
