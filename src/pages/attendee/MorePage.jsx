...
export default function MorePage() {
  ...
  return (
    <>
      <Topbar title="More" actions={
        <Avatar profile={profile} onClick={() => navigate('/app/profile')} />
      } />
      <div className="content">
        {/* Profile summary */}
        <div className="card accent" style={{ marginBottom: 20, display: 'flex', gap: 12, alignItems: 'center' }}>
          <Avatar profile={profile} size={48} onClick={() => navigate('/app/profile')} />
          <div>
        ...
        {ITEMS.map(item => (
          <div key={item.label} className="tile" onClick={() => navigate(item.to)}>
        ...
        <button className="btn btn-danger btn-full" style={{ marginTop: 20 }} onClick={signOut}>
          Sign Out
        </button>
      </div>
    </>
