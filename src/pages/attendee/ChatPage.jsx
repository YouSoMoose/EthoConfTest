...
export default function ChatPage() {
  ...
  useEffect(() => {
    load()
    const ch = sb.channel('chat-updates')
  ...
  async function load() {
    ...
    await sb.from('messages').update({ read: true })
      .eq('to_user_id', profile.id).eq('read', false)
  ...
  async function send() {
    ...
    const recentMine = messages.filter(
      m => m.from_user_id === profile.id && Date.now() - new Date(m.created_at).getTime() < 3600000
    )
  ...
  const chatItems = messages.filter(m =>
    m.to_user_id === profile.id ||
    m.from_user_id === profile.id ||
    m.to_user_id === 'broadcast'
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        {chatItems.length === 0 && (
          <div className="empty">
            ...
            <div className="empty-txt">Ask us anything! We're here to help.</div>
          </div>
        )}
        {chatItems.map(m => {
              ...
              <div className={`bubble-wrap ${isMe ? 'me' : 'them'}`}>
                {(isBroadcast || !isMe) && (
                  <div className="bubble-name">
                    {isBroadcast ? '📣 Ethos Staff' : m.from_name || 'Staff'}
                  </div>
                )}
      ...
      <div style={{
        ...
        <textarea
          ...
          style={{ flex: 1, resize: 'none', minHeight: 44, maxHeight: 120, padding: '10px 12px', fontSize: 14 }}
          placeholder="Message…"
          value={input}
          onChange={e => {
        ...
        />
        <button className="btn btn-accent btn-sm" onClick={send} disabled={sending || !input.trim()}>
          Send
        </button>
      </div>
    </div>
