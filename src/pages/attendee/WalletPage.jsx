...
export default function WalletPage() {
  ...
  useEffect(() => {
    sb.from('collected_cards').select('*').eq('collected_by', profile.id).order('created_at', { ascending: false })
      ...
      <div className="content">
        ...
        {!loading && cards.length === 0 && (
          <div className="empty">
            ...
            <div className="empty-txt">No cards yet — scan someone's QR to save their info!</div>
          </div>
        )}
        {cards.map(c => (
          <div key={c.id} className="card" style={{ marginBottom: 8 }}>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <div style={{
                ...
                background: strColor(c.name || ''),
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 16, fontWeight: 700, color: '#fff', fontFamily: 'var(--fh)',
                flexShrink: 0,
              }}>
                {initials(c.name)}
              </div>
