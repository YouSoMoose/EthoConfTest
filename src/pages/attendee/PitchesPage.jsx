...
export default function PitchesPage() {
  ...
  useEffect(() => {
    Promise.all([
      sb.from('companies').select('*').eq('type', 'pitch').order('name'),
      sb.from('votes').select('company_id').eq('voter_id', profile.id),
    ]).then(([{ data: c }, { data: v }]) => {
      setCompanies(c || [])
    ...
    <>
      ...
      <div className="content">
        ...
        {filtered.map(c => (
          <div
            key={c.id}
            className={`pitch-card ${myVotes.includes(c.id) ? 'voted' : ''}`}
            onClick={() => navigate(`/app/pitches/${c.id}`)}
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 8 }}>
              <div className="pitch-logo" style={{ background: strColor(c.name || '') }}>
                {c.logo_url
                  ? <img src={c.logo_url} alt={c.name} />
                  : <span style={{ color: '#fff', fontFamily: 'var(--fh)', fontWeight: 800 }}>{(c.name || '?')[0]}</span>
                }
              </div>
              ...
              <button
                style={{ background: 'none', border: 'none', color: 'var(--muted)', cursor: 'pointer', padding: 4, fontSize: 18, flexShrink: 0 }}
                onClick={e => { e.stopPropagation(); setQrCompany(c) }}
                title="Show QR"
              >⊞</button>
            </div>
            {c.description && (
        ...
        {!loading && filtered.length === 0 && (
          <div className="empty">
            ...
            <div className="empty-txt">No pitches found</div>
          </div>
        )}
      </div>
