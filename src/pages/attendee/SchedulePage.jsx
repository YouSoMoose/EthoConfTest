...
export default function SchedulePage() {
  ...
  useEffect(() => {
    sb.from('schedule_items').select('*').order('start_time')
    ...
    <>
      ...
      <div className="content">
        ...
        {Object.entries(grouped).map(([day, evts]) => (
          <div key={day}>
            <div style={{
              fontFamily: 'var(--fh)', fontWeight: 700, fontSize: 16,
              marginBottom: 10, marginTop: 16, paddingBottom: 6,
              borderBottom: '2px solid var(--accent)',
            }}>
              {day}
            </div>
            {evts.map(item => (
              <div key={item.id} className="sched-item" onClick={() => navigate(`/app/schedule/${item.id}`)}>
